...
from gui_data import i18n as _i18n
_i18n.init()
...
class AudioTools():
    ...
    def match_inputs(self, audio_inputs, audio_file_base, command_Text):
        import matchering as match  # 延迟导入：只有用「Matchering」工具时才加载
...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def set_interface_language(self):
        """保存界面语言选择（重启后生效）"""
        code = _i18n.code_from_display(self.language_var.get())
        if code and code != _i18n.get_language():
            _i18n.set_language_preference(code)
            messagebox.showinfo(parent=root, title=LANGUAGE_TEXT, message=LANGUAGE_RESTART_MSG)
    ...
    def fill_gpu_list(self):
        ...
        # 检测到 N 卡时自动选中第一块显卡，省去手动设置
        try:
            if cuda_available and torch.cuda.device_count() > 0 and self.device_set_var.get() == DEFAULT:
                self.device_set_var.set(self.cuda_device_list[1])
        except Exception:
            pass
    ...
    def menu_settings(self, select_tab_2=False, select_tab_3=False):#**
        ...
        language_Label.grid(pady=MENU_PADDING_1)
        
        language_Option = ComboBoxMenu(settings_menu_main_Frame, textvariable=self.language_var, values=_i18n.display_names(), width=GEN_SETTINGS_WIDTH+1)
        language_Option.grid(padx=20,pady=MENU_PADDING_1)
        language_Option.bind('<<ComboboxSelected>>', lambda e:self.set_interface_language())
        self.help_hints(language_Label, text=LANGUAGE_HELP)
        ...
        is_half_precision_Option = ttk.Checkbutton(settings_menu_format_Frame, text=HALF_PRECISION_TEXT, width=GEN_SETTINGS_WIDTH, variable=self.is_half_precision_var)
        is_half_precision_Option.grid()
        self.help_hints(is_half_precision_Option, text=IS_HALF_PRECISION_HELP)
    ...
    def load_saved_vars(self, data):
        ...
        self.language_var = tk.StringVar(value=_i18n.display_name(_i18n.get_language()))
