...
> **非官方补丁版 / Unofficial patched build**
>
> 本仓库是 [Ultimate Vocal Remover GUI](https://github.com/Anjok07/ultimatevocalremovergui) v5.6.0 的补丁快照，
> 针对 **RTX 50 系（sm_120）显卡**、新版 Pillow / torch 做了兼容性修复，并新增中文界面。
> 原项目代码版权归 Anjok07 / aufr33 及各位贡献者所有，遵循 MIT 许可（见 [LICENSE](LICENSE)）。
...
- 同时修掉了 Pillow >= 10 导致的「双击没反应」、torch >= 2.6 的 `weights_only` 加载失败、启动黑框等问题。
- 新增**中文界面**（设置里切换）和**半精度开关**（可选，约快 1.9 倍）。
- **启动提速**：`pytorch_lightning` / `matchering` / `onnx2pytorch` 改成按需导入，
  双击到窗口出现从约 6 秒降到 **约 3 秒**。
- **首次推理不用再等 1 分钟**：安装包里带好了 RTX 50 系预热过的 CUDA 内核缓存，
  并把缓存上限调到 2 GB（NVIDIA 默认只有 32 MB，存不下每次都要重编译）。
- ONNX 版 MDX 模型也走 GPU：换成与 CUDA 12.8 匹配的 `onnxruntime-gpu 1.24.4`
  （官方带的 1.29 需要 CUDA 13，会悄悄退回 CPU）。

完整补丁清单见 [PATCHES.md](PATCHES.md)，版本说明见 [RELEASE-NOTES.md](RELEASE-NOTES.md)。

## 支持的显卡

内置 torch 的编译架构：`sm_75 / sm_80 / sm_86 / sm_90 / sm_100 / sm_120`

| 显卡 | 计算能力 | 状态 |
|---|---|---|
| RTX 50 系（5090 / 5080 / 5070 Ti / 5070 / 5060 Ti / 5060，含笔记本） | sm_120 | ✅ 原生 |
| RTX 40 系 | sm_89 | ✅ 走 sm_86 kernel |
| RTX 30 系 / A 系列 | sm_86 / sm_80 | ✅ 原生 |
| RTX 20 系 / GTX 16 系 | sm_75 | ✅ 原生 |

需要 NVIDIA 驱动 **570 以上**。非 N 卡会退回 CPU。
...
git clone <本仓库地址>
cd uvr5-rtx50
python -m venv ..\uvr5-env
..\uvr5-env\Scripts\python.exe -m pip install -r requirements.txt
..\uvr5-env\Scripts\python.exe UVR.py
```

模型权重**不随仓库分发**，请从官方渠道下载后放进 `models/` 对应目录。
