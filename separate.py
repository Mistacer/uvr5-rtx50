...
# ---- RTX 50 系列补丁：可选的 FP16 半精度推理 ----
USE_HALF_PRECISION = False


def set_half_precision(enabled):
    """由 GUI 的「半精度 (FP16)」复选框设置。"""
    global USE_HALF_PRECISION
    USE_HALF_PRECISION = bool(enabled)


def _uvr5_amp(dev):
    """GPU 且开启半精度时返回 autocast 上下文，否则返回空上下文。"""
    try:
        if USE_HALF_PRECISION and torch.cuda.is_available() and str(dev).startswith('cuda'):
...
class SeperateAttributes:
    def __init__(self, model_data: ModelData, 
        ...
        self.device = cpu
        set_half_precision(process_data.get('is_half_precision', False))
...
class SeperateMDX(SeperateAttributes):        
    ...
    def seperate(self):
        ...
        if self.primary_model_name == self.model_basename and isinstance(self.primary_sources, tuple):
            ...
            if self.is_mdx_ckpt:
                import lib_v5.mdxnet as MdxnetSet  # 延迟导入：pytorch_lightning 只有 .ckpt 模型才用
                ...
                if self.mdx_segment_size == self.dim_t and not self.is_other_gpu:
                    ...
                    from onnx import load  # 延迟导入：只有 ONNX 转 PyTorch 时才加载
    ...
    def demix(self, mix, is_match_mix=False):
        ...
        for i in range(0, mixture.shape[-1], step):
            ...
            with torch.no_grad(), _uvr5_amp(self.device):
...
class SeperateVR(SeperateAttributes):        
    ...
    def inference_vr(self, X_spec, device, aggressiveness):
        def _execute(X_mag_pad, roi_size):
            ...
            self.model_run.eval()
            with torch.no_grad(), _uvr5_amp(device):
                mask = []
